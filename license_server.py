...
# ── Add to license_server/main.py ────────────────────────────────────────

class BackupMetaRequest(BaseModel):
    tenant_id:    int
    filename:     str
    size_mb:      float
    drive_file_id: str
...
@app.post("/api/backup/register")
async def register_backup(body: BackupMetaRequest):
    """Called by the desktop app after every successful Google Drive upload."""
    ...
    """)
    cur.execute(
        """INSERT INTO backup_records (tenant_id, filename, size_mb, drive_file_id)
           VALUES (%s, %s, %s, %s)""",
        (body.tenant_id, body.filename, body.size_mb, body.drive_file_id),
    )
    conn.commit()
    cur.close()
    conn.close()
    return {"success": True, "message": "Backup registered."}


@app.get("/api/backup/list/{tenant_id}")
async def list_backups(tenant_id: int, admin_secret: str):
    """Return all cloud backups for a tenant."""
    if admin_secret != os.getenv("ADMIN_SECRET", "my-admin-secret"):
        raise HTTPException(status_code=403, detail="Forbidden")
    ...
    try:
        cur.execute("""
            SELECT filename, size_mb, drive_file_id, created_at
            FROM backup_records
            WHERE tenant_id = %s
